...
var dbForUserDetails = cloudant.db.use("userdetails");
var dbForRequestDetails = cloudant.db.use("requestdetails"); 
...
app.post('/userInfo', type , function (req, res) {
console.log("Got a POST request for UserDetails.html page");
   console.log(req);
   var uniqueId = req.body.name.substring(0, 3) + req.body.dob.substring(0, 2) + req.body.ssn.substring(0,3);
   console.log(uniqueId);
   fs.readFile(__dirname + '/upload/' + req.file.filename, function(err, data) {
	  if (!err) {
		console.log(data);
		dbForUserDetails.multipart.insert(req.body, [{name: req.file.originalname, data: data, content_type: req.file.mimetype}], uniqueId , function(err, body) {
			if (!err){
			  console.log(body);
	...
	dbForRequestDetails.list(function(err, body) {
	 ...
	 console.log(requestId);
		...
		dbForRequestDetails.insert(requestDetails, requestId, function(err, body) {
		  if (!err)
			console.log(body);
		});
	});
